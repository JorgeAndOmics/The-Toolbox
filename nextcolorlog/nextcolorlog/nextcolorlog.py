...
class ColoredLogger:
    ...
    class SeparatorFilter(logging.Filter):
        """
        A custom logging filter to add a separator to log records.
        """
        def filter(self, record):
            '''
            Modify each log record to include a custom separator.

            Args:
                record (logging.LogRecord): The log record to be modified.

            Returns:
                bool: True to indicate that the record should be processed.
            '''
            record.separator = '-'
            return True
